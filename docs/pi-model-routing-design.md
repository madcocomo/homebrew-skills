...
本设计通过一个自包含的 Pi Extension，在安全时临时租用静态配置的弱模型；复杂任务始终保留用户选择的当前模型：
...
确定性规则允许
    ...
    AND capsule 完整
    AND 图片能力匹配
    AND 未触发 lease-release signal
        => 临时 weak lease
否则 => 不干预用户当前模型
```

只有分类器和弱模型由配置固定指定；不存在 `models.strong`、候选池、健康探针或动态换模。
...
3. 对弱模型执行结果持续评估，在失败、漂移或无进展时释放 weak lease，并恢复进入 lease 前捕获的用户模型。
...
- 不让分类器成为唯一安全门。
- 不为 classifier/weak 自动选择“当前可用的相近模型”，也不配置或选择独立 strong 模型。
...
| Lease-release signal | 使 weak lease 终止并恢复其 return model 的执行信号。 |
...
6. **Route state machine**：维护 mode、activation snapshot、request/turn、weak lease 及每个 lease 的 return model。
7. **Tool observer/evaluator**：观察调用与结果，不阻止工具，仅产生效果和 lease-release signal。
...
routeState             undecided | weak-lease
leaseReturnModel        进入当前 weak lease 前捕获的精确用户模型对象
...
releaseSignals         当前 run 已发现的 lease-release signal
lastTargetModel         最近目标模型
lastActualModel         最近 Assistant 实际模型
```

只把恢复和审计所需的最小状态通过 `pi.appendEntry("model-router-state", ...)` 写入 session：runtime mode、activation model、request id、route state 和最近释放原因。完整模型对象、prompt、工具输出和密钥不进入 custom entry；lease return model 仅保存在当前进程内。
...
[active-ready] ←──────────┘                         │
  │ initial weak                                    │
  ▼                                                 │
[weak-lease] ── release signal / turn cap ──→ restore lease return model
  │ healthy tool batch                               │
  └──────────────────→ [weak-lease]                  │
                                                    │
任意非 off 状态 ── /routing off ──→ restore activation model ──→ [off]
active/weak ── /routing shadow ──→ restore activation model ──→ [shadow-ready]
```

关键不变量：

- 每个 weak lease 在切 weak 前捕获精确 return model；正常结束或任一 release signal 都恢复该模型。
- 新用户请求开始前若仍有遗留 weak lease，先恢复其 return model，再重新决策。
- shadow 从不调用 `pi.setModel()`。
- off 不执行分类器、不写路由日志、不更新路由状态栏。
- active 只允许切到配置的 weak，或恢复当前 lease 捕获的用户模型；不得切到配置的 strong/fallback。
...
3. 清除 weak lease、operation fingerprints、capsule、release signals 和状态栏。
...
8. active 仅在 weak verdict 时捕获当前用户模型为 lease return model，再调用 `pi.setModel(weak)`；weak 不可用则保持当前模型，不尝试 fallback。strong verdict 不调用 `setModel()`；若前一 lease 尚未释放，先恢复其 return model。
...
Weak model ─→ tool calls ─→ tool_result observers ─→ turn_end
                                                  │
                                                  ├─ no tools + normal: agent_end restores return model
                                                  │
                                                  ├─ healthy: keep weak lease
                                                  │
                                                  └─ failure/signal/cap: restore return model
                                                                      │
                                                                      ▼
...
4. 执行效果评估。
5. 若当前为 weak-lease 且健康、未到上限，保持 weak。
6. 若触发任一 release signal，在 handler 返回前恢复进入 lease 前捕获的用户模型，使下一 provider request 不再使用 weak。
7. weak provider 返回 `error`/`aborted` 时即使没有工具 batch，也立即释放 lease。
8. `agent_end` 总是释放仍存活的 weak lease；恢复失败明确 warning、结束 lease，且不尝试任何 fallback 模型。

Continuation 不重复调用分类器。这样既避免每个工具 turn 额外增加约 11～13 秒，也将 weak 限定为有明确起止点的临时 lease。
...
解析失败、超时、abort、模型缺失、凭据缺失或 provider error 均得到 `classifier_failure`，路由器不干预用户当前模型。不会尝试第二个 classifier。
...
- 对 bash/Git，识别明确的 cwd、`git -C`、`cd` 和已知产物路径；无法可靠解析时不伪装为“安全”，产生 `scope_observation_uncertain` release signal。
- 漂移检测只观察并在下一 request 前释放 weak lease，不基于工具名拦截当前工具。
...
- agent run 结束时若要求的验收从未执行或产物缺失，日志标记 `expected_acceptance_hit=false`；若还有 continuation，则立即释放 weak lease。

---

## 14. Weak lease 释放策略

任一信号触发恢复当前 lease 的 return model：
...
释放动作使用每个 lease 自己的快照：

1. 切 weak 前，从 `ctx.model` 捕获精确模型对象为 `leaseReturnModel`。
2. 追加 signal 到当前 request 状态并将 lease 标记为结束，保证恢复失败时也不重复尝试。
3. `await pi.setModel(leaseReturnModel)`，并在 handler 返回前完成。
4. 正常 `agent_end` 也执行同一释放动作，避免下一个用户请求继承 weak。
5. 恢复失败时明确 warning，不尝试任何 fixed strong、候选或 fallback 模型。

新请求若发现遗留 weak lease，会先执行释放，再进行本次 admission/classification。用户在两次任务之间手工选择的新模型，会成为下一次 weak lease 的新 return model。
...
|---|---:|---|
| 否 | 任意 | 走普通规则。 |
| 是 | 是 | 仍需普通规则和 classifier；可 weak。 |
| 是 | 否 | strong verdict：不调用 classifier，不干预用户当前模型。 |
...
Continuation 记录相同关联 id，并更新工具摘要、actual model、usage 和 acceptance；release signal 只进入运行时状态和受控 reason code，不记录自由文本。
...
- 图片任务仅当 fixed weak 支持图片且 capsule 能明确提供图片输入时才允许；首版可以保守拒绝 child 图片委派并留在父任务的用户模型。
...
Extension validates exit/artifacts/acceptance
   ...
   └ failure -> isError + parent continuation releases its weak lease
```
...
| 命令 | 行为 |
|---|---|
| `status` | 显示配置路径、configured/effective mode、classifier/weak、activation model、当前 lease、最近 reason/signal、日志目录、sub-pi 是否启用。 |
| `shadow` | 捕获 activation model；验证配置；只分类/记录；若从 active 进入则先恢复 activation model。 |
| `active` | 捕获 activation model；严格校验 classifier/weak 可解析且有凭据；启用仅向 weak 降级的临时 lease。 |
| `off` | 等待 idle、恢复 activation model、清理状态/状态栏，停止分类和日志副作用。 |

状态栏示例：

```text
routing:shadow · target=weak · actual=user-model
routing:active · weak · turn=2/4
routing:active · user-model · released=verification_failed
routing:error · lease_restore_failed
```
...
| classifier model 缺失/无凭据/超时/调用失败 | 不干预用户当前模型；记录 `classifier_failure`。 |
| weak model 缺失或 `setModel=false` | 保持用户当前模型；记录 `weak_unavailable`；不 fallback。 |
| 模型声明支持图片但 registry 不支持 | 配置错误；不得 active。 |
| weak provider request 失败 | 在 `turn_end` 恢复 lease return model；不尝试其他 weak。 |
| lease return model 恢复失败 | 明确 warning、结束 lease；不尝试其他模型。 |
| 日志目录不可写/磁盘满 | 限频 warning；继续普通 Pi 和路由。 |
| classifier 输出非法 JSON | 不干预用户当前模型；保留结构化错误码，不记录原始响应全文。 |
...
| sub-pi 非零/超时/验收失败 | 工具结果失败；父 continuation 释放其 weak lease。 |
...
3. **保守失败**：classifier/weak 失败时保留或恢复用户模型；不选择 fallback。
4. **规则先于模型**：hard-strong/reject 不可被 classifier 覆盖；其中 strong 表示不干预。
5. **不拦工具但持续评估**：不因工具名阻止弱模型；通过 capsule、结果和 lease 释放控制风险窗口。
...
新增：`/routing active`、activation snapshot/restore、首轮 weak 切换、每个 lease 的 return-model 快照、continuation 评估、自动释放、图片路由、状态栏和 model change 验证。
...
- classifier 协议有效响应率至少 99%；失败项全部可观察地得到 strong verdict（不干预）。
...
- fake harness 证明 `turn_end` 的 lease restore 在下一 provider request 前完成。
- 注入的非零退出、tool error、scope drift、重复操作、产物缺失、验收失败、无进展和 turn cap 均在下一 request 前恢复当前 lease 的 return model。
- weak lease 正常 `agent_end`、执行失败或任一 release signal 后恢复进入 lease 前的用户模型。
- shadow 模式 `setModel` 调用次数严格为 0。
- `/routing off` 恢复 activation model 并清理状态；恢复失败明确可见。
- active 试运行中验收通过率不得低于“始终使用用户模型”基线；应报告用户模型 provider request 降幅，初始目标为至少 25%，但正确性 gate 优先。
...
- fake child 的非零、超时、越界、产物缺失和验收失败都返回失败并使父 continuation 释放 weak lease。
...
- smoke 只使用配置中的 classifier/weak；strong verdict 保留用户模型，不尝试任何候选模型。
